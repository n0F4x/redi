...
export module redi.util.containers.Any;
...
export template <typename T>
using DefaultAnyPolicy = AlwaysTrue<T>;

export template <decayed_c T, typename Any_T>
    requires storable_in_any_c<T, std::remove_cvref_t<Any_T>>
[[nodiscard]]
constexpr auto any_cast(Any_T&& any) -> forward_like_t<T, Any_T>;

export template <decayed_c T, typename Any_T>
    requires any_c<std::remove_cvref_t<Any_T>>
[[nodiscard]]
constexpr auto reinterpret_any_cast(Any_T&& any) -> forward_like_t<T, Any_T>;
...
public:
    ...
    template <decayed_c T, typename Any_T>
        requires storable_in_any_c<T, std::remove_cvref_t<Any_T>>
    friend constexpr auto any_cast(Any_T&& any) -> forward_like_t<T, Any_T>;

    template <decayed_c T, typename Any_T>
        requires any_c<std::remove_cvref_t<Any_T>>
    friend constexpr auto reinterpret_any_cast(Any_T&& any) -> forward_like_t<T, Any_T>;
...
constexpr auto any_cast(Any_T&& any) -> forward_like_t<T, Any_T>
{
  ...
  #pragma clang diagnostic pop
    ...
    PRECOND(
        any.NakedAny::BasicAny::m_erase_mechanism.type_hash() == util::hash_u64<T>(),
        std::format(
            "`Any` has type {}, but requested type is {}",
            any.NakedAny::BasicAny::m_erase_mechanism.type_name(),
            util::name_of<T>()
        )
    );

    auto* const address{ any.NakedAny::BasicAny::m_erase_mechanism
    ...
    return std::forward_like<Any_T>(
        ...
        *static_cast<const_like_t<T, std::remove_reference_t<Any_T>>*>(address)
    );
}
...
constexpr auto reinterpret_any_cast(Any_T&& any) -> forward_like_t<T, Any_T>
{
  ...
  #pragma clang diagnostic pop
    ...
    return std::forward_like<Any_T>(
        // ReSharper disable once CppDFANullDereference
        *reinterpret_cast<const_like_t<T, std::remove_reference_t<Any_T>>*>(address)
    );
}
